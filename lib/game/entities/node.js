...
EntityNode = ig.Entity.extend({
		...
		if( ig.input.released( 'action' ) ) {
			...
			if( this.dragStart ) {
				if( this.dragType === 'core' ) {
					this.drawCoreDragUi = false;
					if( this.canExpand ) {
						ig.game.spawnEntity( 'EntityNode', ig.input.mouse.x, ig.input.mouse.y, { team: this.team } );
						this.data -= this.cost;
					}
				}
				if( this.dragType === 'virus' ) {
	...
	draw: function() {
		...
		if( this.drawVirusDragUi ) {
			...
			this.drawCross();
			this.drawValueTooltip( Color.gray, this.cost === 0 ? '' : this.cost, true );
