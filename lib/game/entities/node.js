...
EntityNode = ig.Entity.extend({
	...
	drawValueTooltip: function( color, value, viral ) {
		...
		var x = (dist+32) * Math.cos( angle + (90).toRad() ) + (this.dragPos.x-this.pos.x);
		var y = (dist+32) * Math.sin( angle + (90).toRad() ) + (this.dragPos.y-this.pos.y);
	...
	drawUI: function() {
		...
		if( this.drawCoreDragUi && ig.input.state( 'action' ) && !this.threadMenu && dist > 5 ) {
			...
			ig.system.context.beginPath();
			ig.system.context.arc( pos.x, pos.y, 128, 0, Math.PI*2, true ); 
			ig.system.context.closePath();
			ig.system.context.stroke();
