...
EntityNode = ig.Entity.extend({
			...
			if( this.dragStart ) {
				...
				if( this.dragType === 'virus' ) {
					if( this.canExpand && distCenter > r1 ) {
						this.attacking = true;
						this.attackPoints = new Float32Array( ((this.cost/2)<<0)*2 );

						var sin;
						slice = 360/((this.cost/2)<<0);
						for( var i = 0; i < ((this.cost/2)<<0)*2; i+=2 ) {
							x = 15 * Math.cos( (slice*i).toRad() );
							y = 15 * Math.sin( (slice*i).toRad() );
							console.log( i );
							//sin = Math.sin( new Date().getTime()/500 + (i*200) ) * 10;
							this.attackPoints[i] = x;
							this.attackPoints[i+1] = y;
						}
	...
	draw: function() {
		...
		if( this.attacking ) {
			...
			for( var i = 0; i < this.attackPoints.length; i+=2 ) {
				ctx.beginPath();
				ctx.globalAlpha = 0.5;
				ctx.arc( this.attackPoints[i], this.attackPoints[i+1], 4, 0, Math.PI*2, true ); 
				ctx.closePath();
				ctx.fill();
			}
			...
			ig.system.context.drawImage( this.backbuffer, ig.input.mouse.x-w, ig.input.mouse.y-w );
		}
	}
});
