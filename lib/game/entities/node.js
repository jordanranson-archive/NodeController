...
EntityNode = ig.Entity.extend({
	...
	update: function() {
		...
		ig.show( 'm', (ig.game.mouse.x).toFixed()+','+(ig.game.mouse.y).toFixed() );
	},
	...
	draw: function() {
		...
		if( this.drawCoreDragUi && ig.input.state( 'action' ) && !this.threadMenu && dist > 5 ) {
			...
			ig.system.context.beginPath();
			ig.system.context.arc( ig.game.mouse.x, ig.game.mouse.y, 128, 0, Math.PI*2, true ); 
			ig.system.context.closePath();
			ig.system.context.stroke();
			...
			ig.system.context.lineTo( ig.game.mouse.x, ig.game.mouse.y );
			ig.system.context.closePath();
			ig.system.context.stroke();
