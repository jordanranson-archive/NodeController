...
EntityNode = ig.Entity.extend({
	...
	update: function() {
		...
		var r2 = Math.min((this.vaccineRad),125) + r1;
		var r3 = Math.min((this.virusRad),200) + r2;
		this.radius = r3;
		...
		if( this.team === ig.game.playerColor ) {
			...
			if( ig.input.pressed( 'action' ) && ig.game.playerAttacking && this !== ig.game.attackingNode ) {
				if( this.distanceToPoint( ig.input.mouse ) < r1 ) {
		...
		if( ig.input.state( 'action' ) ) {
			...
			if( this.drawCoreDragUi || this.drawVirusDragUi ) {
				...
				if( this.dragType === 'core' && distCenter > r3+128 ) {
					...
					this.cost = this.virus*(dist/this.radius);
		...
		if( this.distanceToPoint( ig.input.mouse ) < r1 ) {
			...
			if( !ig.input.state( 'action' ) && ig.game.playerAttacking && ig.game.attackingNode !== this ) {
	...
	draw: function() {
		...
		ig.system.context.globalAlpha = this.virusAlpha.map( 0, 1, 0, this.threadAlpha );
		ig.system.context.fill();
		...
		ig.system.context.globalAlpha = this.uiCurAlpha.map( 0, 1, 0, ringAlpha );
		...
		if( ig.game.playerAttacking && ig.game.attackingNode !== this ) {
		...
		if( this.attacking ) {
			...
			ig.system.context.beginPath();
			ig.system.context.globalAlpha = this.canAttack ? 0.45 : 0.15;
			...
			for( var i = 0; i < this.attackPoints.length; i+=5 ) {
				...
				ig.system.context.beginPath();
				ig.system.context.globalAlpha = this.attackPoints[i+4] ? this.attackPoints[i+4].map( 0, 1, 0, alpha ) : 0;
				ig.system.context.arc( this.attackPoints[i], this.attackPoints[i+1], size, 0, Math.PI*2, true ); 
				ig.system.context.closePath();
				ig.system.context.fill();
			}
