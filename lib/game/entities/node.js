...
EntityNode = ig.Entity.extend({
	...
	drawCross: function() {
		...
		ig.system.context.moveTo( ig.game.mouse.x-7, ig.game.mouse.y-7 );
		ig.system.context.lineTo( ig.game.mouse.x+7, ig.game.mouse.y+7 );
	...
	drawArrow: function() {
		...
		ig.system.context.translate( ig.game.mouse.x, ig.game.mouse.y );
	...
	draw: function() {
		...
		if( this.drawCoreDragUi && ig.input.state( 'action' ) && !this.threadMenu && dist > 5 ) {
			...
			ig.system.context.beginPath();
			ig.system.context.arc( ig.game.mouse.x-this.pos.x, ig.game.mouse.y-this.pos.y, 128, 0, Math.PI*2, true ); 
			ig.system.context.closePath();
			ig.system.context.stroke();
			...
			ig.system.context.lineTo( ig.game.mouse.x-this.pos.x, ig.game.mouse.y-this.pos.y );
			ig.system.context.closePath();
			ig.system.context.stroke();
		...
		if( this.drawVirusDragUi && ig.input.state( 'action' ) && !this.threadMenu && dist > 5 ) {
			...
			ig.system.context.beginPath();
			ig.system.context.moveTo( this.dragPos.x-this.pos.x, this.dragPos.y-this.pos.y );
			ig.system.context.lineTo( ig.game.mouse.x-this.pos.x, ig.game.mouse.y-this.pos.y );
			ig.system.context.closePath();
			ig.system.context.stroke();
		...
		ctx.save();
		ctx.rotate( (15).toRad() );
		ctx.lineWidth = 2/ig.game.zoom;

		// virus

		ctx.strokeStyle = Color.black;
		ctx.beginPath();
		ctx.moveTo( -r3 + ((r3-r2)*0.5) + 2/ig.game.zoom, 2/ig.game.zoom );
		ctx.lineTo( -r3 + ((r3-r2)*0.5) + 2/ig.game.zoom - Math.max(40, r3-r2), 2/ig.game.zoom );
		ctx.closePath();
		...
		ctx.strokeStyle = Color.black;
		ctx.beginPath();
		ctx.moveTo( r2 - ((r2-r1)*0.5) + 2/ig.game.zoom, 2/ig.game.zoom );
		ctx.lineTo( r2 - ((r2-r1)*0.5) + 2/ig.game.zoom + Math.max(60, r2-r1), 2/ig.game.zoom );
		ctx.closePath();
		ctx.globalAlpha = this.uiCurAlpha.map( 0, 1, 0, 0.35 )*this.globalAlpha;
		ctx.stroke();
		...
		if( ig.game.playerAttacking && ig.game.attackingNode !== this && this.team !== ig.game.playerColor && this.alive && ig.game.attackingNode.distanceToPoint( this.pos ) < ig.game.attackingNode.radius*3 ) {
			...
			ctx.strokeStyle = Color.black;
			ctx.beginPath();
			ctx.moveTo( 2/ig.game.zoom, -r1 + ((r2-r1)*0.5) + 2/ig.game.zoom );
			ctx.lineTo( 2/ig.game.zoom, -r3 + ((r3-r2)*0.5) + 2/ig.game.zoom - Math.max(40, r3-r2) );
			ctx.closePath();
			ctx.globalAlpha = this.uiCurAlpha.map( 0, 1, 0, 0.35 )*this.globalAlpha;
			ctx.stroke();
		...
		if( ig.game.playerAttacking && ig.game.attackingNode !== this && this.team === ig.game.playerColor && this.alive ) {
			...
			ctx.lineTo( 0, -r3 + ((r3-r2)*0.5) - Math.max(40, r3-r2) );
			ctx.closePath();
			ctx.globalAlpha = this.uiCurAlpha.map( 0, 1, 0, 0.1 )*this.globalAlpha;
			ctx.stroke();
