...
NodeController = ig.Game.extend({
	...
	init3d: function() {
		...
		// set up viewport boundries
	    var w = (ig.game.renderer.domElement.width/4)*0.1;
	    var h = (ig.game.renderer.domElement.width/4)*0.15;
	    var x = ((ig.game.renderer.domElement.width*0.5)/4)-(w*0.5);
	    var y = ((ig.game.renderer.domElement.width*0.5)/4)-(h*0.5);
	    this.viewport = {};
	    this.viewport.x1 = -x+(w*0.5);
	    this.viewport.y1 = y-(h*0.5);
	    this.viewport.x2 = x-(w*0.5);
	    this.viewport.y2 = -y+(h*0.5);
	    ...
	    for( var i = 0; i <= 80; i++ ) {
	    	...
	    	if( dir ) {
	    		...
	    		geometry.vertices.push( new THREE.Vector3(80*16, i*16, 0) );
	    	}
	    	else {
	    		geometry.vertices.push( new THREE.Vector3(80*16, i*16, 0) );
	    		geometry.vertices.push( new THREE.Vector3(0, i*16, 0) );
	    ...
	    for( var i = 80; i >= 0; i-- ) {
	    	...
	    	if( dir ) {
	    		...
	    		geometry.vertices.push( new THREE.Vector3(i*16, 80*16, 0) );
	    	}
	    	else {
	    		geometry.vertices.push( new THREE.Vector3(i*16, 80*16, 0) );
	    		geometry.vertices.push( new THREE.Vector3(i*16, 0, 0) );
	    ...
	    this.grid.position.x = -640;
	    this.grid.position.y = -640;
	    ...
	    // bezel
	    this.bezel = new THREE.Object3D();
	    var plane;

	    // left
    	plane = new THREE.Mesh(new THREE.PlaneGeometry( 160, 1000 ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
		plane.position.set( this.viewport.x1-80, 0, 0.2 );
	    this.bezel.add( plane );

	    // right
	    plane = new THREE.Mesh(new THREE.PlaneGeometry( 160, 1000 ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
		plane.position.set( this.viewport.x2+80, 0, 0.2 );
	    this.bezel.add( plane );

	    // top
    	plane = new THREE.Mesh(new THREE.PlaneGeometry( 1000, 160 ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
		plane.position.set( 0, this.viewport.y1+80, 0.2 );
	    this.bezel.add( plane );

	    // bottom
    	plane = new THREE.Mesh(new THREE.PlaneGeometry( 1000, 160 ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
		plane.position.set( 0, this.viewport.y2-80, 0.2 );
	    this.bezel.add( plane );

	    // border
	    material = new THREE.LineBasicMaterial({ color: 0x666666 });
	    geometry = new THREE.Geometry();
		geometry.vertices.push( new THREE.Vector3(this.viewport.x1, this.viewport.y1, 0.3) );
		geometry.vertices.push( new THREE.Vector3(this.viewport.x2, this.viewport.y1, 0.3) );
		geometry.vertices.push( new THREE.Vector3(this.viewport.x2, this.viewport.y2, 0.3) );
		geometry.vertices.push( new THREE.Vector3(this.viewport.x1, this.viewport.y2, 0.3) );
		geometry.vertices.push( new THREE.Vector3(this.viewport.x1, this.viewport.y1, 0.3) );
	    ...
	    // add level and bezel to scene
	    this.gameobject.add( this.level );
	    this.gameobject.add( this.bezel );
