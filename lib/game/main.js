...
NodeController = ig.Game.extend({
	...
	init3d: function() {
	    ...
	    var dir, point = -1;
	    this.grid = new Float32Array((40*40)*2);
	    for( var i = 0; i <= 40; i++ ) {
	    	dir = i % 2 === 0;

	    	if( dir ) {
		    	this.grid[point++] = 0;
		    	this.grid[point++] = i*16;
	    		geometry.vertices.push( new THREE.Vector3(this.grid[point-2], this.grid[point-1], 0) );

		    	this.grid[point++] = 40*16;
		    	this.grid[point++] = i*16;
	    		geometry.vertices.push( new THREE.Vector3(this.grid[point-2], this.grid[point-1], 0) );
	    	}
	    	else {
	    ...
	    // test shape
	    this.cube = new THREE.Mesh(new THREE.OctahedronGeometry( 8, 0 ), new THREE.MeshNormalMaterial())
	...
	update: function() {
		...
		if( ig.input.pressed( 'action2' ) ) {
            ...
            this.dragStart = {};
            this.dragStart.x = this.camera.position.x;
            this.dragStart.y = this.camera.position.y;
        }
        if( ig.input.state( 'action2' ) ) {
            var x = ig.input.mouse.x - this.mouseStart.x;
            var y = ig.input.mouse.y - this.mouseStart.y;

            this.camera.position.x = (this.dragStart.x - x);
            this.camera.position.y = (this.dragStart.y - (y*-1));
        }

        if( ig.input.state( 'scroll_up' ) ) {
            if( this.camera.position.z > 0 ) {
                this.camera.position.z -= this.camera.position.z*0.035;
            }
        }
