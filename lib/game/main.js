...
NodeController = ig.Game.extend({
	...
	init3d: function() {
	    ...
	    h = (ig.game.renderer.domElement.height/4);
	    x = ((ig.game.renderer.domElement.width*0.5)/4)-(w*0.5);
	    y = 0;
    	plane = new THREE.Mesh(new THREE.PlaneGeometry( w, h ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
	    ...
	    this.bezel.add( plane );
	    // top
	    w = (ig.game.renderer.domElement.width/4);
	    h = (ig.game.renderer.domElement.height/4)*0.1;
	    x = 0;
	    y = ((ig.game.renderer.domElement.height*0.5)/4)-(h*0.5);
    	plane = new THREE.Mesh(new THREE.PlaneGeometry( w, h ), new THREE.MeshBasicMaterial({ color: 0x222222 }));
		plane.position.set( x, y, 0.1 );
	    this.bezel.add( plane );
	    // bottom
		...
		geometry.vertices.push( new THREE.Vector3(-x+(w*0.5), y-(h*0.5), 0.2) );
		geometry.vertices.push( new THREE.Vector3(x-(w*0.5), y-(h*0.5), 0.2) );
		geometry.vertices.push( new THREE.Vector3(x-(w*0.5), -y+(h*0.5), 0.2) );
		geometry.vertices.push( new THREE.Vector3(-x+(w*0.5), -y+(h*0.5), 0.2) );
		geometry.vertices.push( new THREE.Vector3(-x+(w*0.5), y-(h*0.5), 0.2) );
