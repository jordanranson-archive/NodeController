...
lg.Util.importLevel = function( image ) {
	...
	var getColor = function( data, x, y ) {
		var index = (y * image.width + x) * 4;
		if( data[index] !== undefined && data[index+2] !== undefined ) {
...
lg.Util.drawTilemap = function( map ) {
	...
	for( var y = 0; y < map.length; y++ ) {
		for( var x = 0; x < map[y].length; x++ ) {
			...
			if( tile === 'X' ) context.fillRect( x*ts, y*ts, ts, ts );
			if( tile === '0' ) context.fillRect( x*ts, y*ts+6+1, ts, 2 );
			if( tile === '11' ) {
				context.fillRect( x*ts+6, y*ts+6+1, ts-6, 2 );
			}
			if( tile === '12' ) {
				context.fillRect( x*ts, y*ts+6+1, ts-6, 2 );
			}
			if( tile === '1' ) context.fillRect( x*ts+6+1, y*ts, 2, ts );
			if( tile === '13' ) {
				context.fillRect( x*ts+6+1, y*ts, 2, ts-6 );
			}
			if( tile === '14' ) {
				context.fillRect( x*ts+6+1, y*ts+6+1, 2, ts-6 );
			}
			if( tile === '2' ) {
				context.fillRect( x*ts+6+1, y*ts+6+1, 8+1, 2 );
				context.fillRect( x*ts+6+1, y*ts, 2, 8 );
			}
			if( tile === '3' ) {
				context.fillRect( x*ts, y*ts+6+1, 8+1, 2 );
				context.fillRect( x*ts+6+1, y*ts, 2, 8+1 );
			}
			if( tile === '4' ) {
				context.fillRect( x*ts+6+1, y*ts+6+1, 8+1, 2 );
				context.fillRect( x*ts+6+1, y*ts+6+1, 2, 8+1 );
			}
			if( tile === '5' ) {
				context.fillRect( x*ts, y*ts+6+1, 8, 2 );
				context.fillRect( x*ts+6+1, y*ts+6+1, 2, 8+1 );
			}
			if( tile === '6' ) {
				context.fillRect( x*ts, y*ts+6+1, ts, 2 );
				context.fillRect( x*ts+6+1, y*ts, 2, 8 );
			}
			if( tile === '7' ) {
				context.fillRect( x*ts, y*ts+6+1, ts, 2 );
			...
			if( tile === '8' ) {
				context.fillRect( x*ts+6+1, y*ts+6+1, 8+1, 2 );
				context.fillRect( x*ts+6+1, y*ts, 2, ts );
			}
			if( tile === '9' ) {
				context.fillRect( x*ts, y*ts+6+1, 8, 2 );
				context.fillRect( x*ts+6+1, y*ts, 2, ts );
			...
			// draw nodes
			if( tile === 'N' ) context.fillRect( x*ts, y*ts, ts, ts );
		}
	}
